...
def has_glasses(image,detection):
    h, w, _ = image.shape
    ...
    return glasses.has_glasses(cropped_face)
...
while True:
    ...
    if(face_result.detections):
        if(has_face):
            ...
            for detection in face_result.detections:
                ...
                try:
                    flag = "has_glasses " if has_glasses(frame,detection) else "not "
    ...
    if results.multi_hand_landmarks:
        for idx, hand_landmarks in enumerate(results.multi_hand_landmarks):
            ...
            # 在图像上显示手部类型和方向
            cv2.putText(frame, "h: "+hand + " d: " + direction, (int(x_thumb * frame.shape[1]), int(y_thumb * frame.shape[0])), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2, cv2.LINE_AA)
